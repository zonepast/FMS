...
import com.prudent.fms.utils.Calender.DateListAdapter;
import com.prudent.fms.utils.Calender.DateModel;
import com.prudent.fms.utils.Calender.EventListAdapter;
import com.prudent.fms.utils.Calender.EventModel;
import com.prudent.fms.utils.Calender.GetEventListListener;
...
//        tv_month_year.setTextColor(Color.parseColor(strHeaderTextColor));
    ...
    public void addEvent(String date, String startTime, String endTime, String name,String color) {
        if (!TextUtils.isEmpty(date) && !TextUtils.isEmpty(startTime) && !TextUtils.isEmpty(endTime) && !TextUtils.isEmpty(name)) {
            ...
            AppConstants.eventList.add(new EventModel(tmpDate, tmpStartTime, tmpEndTime, name,color));
        }
    }

    public void addEvent(String date, String startTime, String endTime, String name, String color,int image) {
        if (!TextUtils.isEmpty(date) && !TextUtils.isEmpty(startTime) && !TextUtils.isEmpty(endTime) && !TextUtils.isEmpty(name)) {
            String tmpDate = GlobalMethods.convertDate(date, AppConstants.sdfDate, AppConstants.sdfDate);
            String tmpStartTime = GlobalMethods.convertDate(startTime, AppConstants.sdfHourMinute, AppConstants.sdfHourMinute);
            String tmpEndTime = GlobalMethods.convertDate(endTime, AppConstants.sdfHourMinute, AppConstants.sdfHourMinute);
            AppConstants.eventList.add(new EventModel(tmpDate, tmpStartTime, tmpEndTime, name, color,image));
        }
    }
...
//    -------------------------------------------------------------------------
    ...
    private void setMonthView(String sign) {
        ...
        recyclerView_dates.setNestedScrollingEnabled(true);
            ...
            public void onClick(Date date) {
                ...
                for (int i = 0; i < AppConstants.eventList.size(); i++) {
                    if (AppConstants.eventList.get(i).getStrDate().equals(AppConstants.sdfDate.format(date))) {
                        eventModelList.add(new EventModel(AppConstants.eventList.get(i).getStrDate(), AppConstants.eventList.get(i).getStrStartTime(), AppConstants.eventList.get(i).getStrEndTime(), AppConstants.eventList.get(i).getStrName(),AppConstants.eventList.get(i).getStrColor()));
                    }
                }
